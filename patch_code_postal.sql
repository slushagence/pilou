-- Ajouter la colonne code_postal
ALTER TABLE parties ADD COLUMN IF NOT EXISTS code_postal text;

-- Mettre à jour la fonction jouer() pour accepter p_code_postal
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE lower(email) = lower(p_email) AND jour = v_jour AND lieu_id = p_lieu_id
  ) THEN
